...
<div class="modal fade" id="accountModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><form class="modal-content" id="accountForm"><div class="modal-header"><h5 class="modal-title">Add account</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body">
    ...
    <div class="row g-3 mt-0"><div class="col-md-6"><label class="form-label">Last 4 digits</label><input class="form-control" name="last_four" maxlength="4" inputmode="numeric"></div><div class="col-md-6 create-only"><label class="form-label">Opening balance</label><input class="form-control" name="opening_balance" type="number" min="0" step="0.01"></div></div>
    <div class="create-only mt-3"><label class="form-label">Opening balance date</label><input class="form-control" name="opening_balance_date" type="date" value="{{ now()->toDateString() }}"></div>
...
<script>
(() => {
    const M=MyLedger, cards=document.querySelector('#accountCards'), form=document.querySelector('#accountForm'); let includeArchived=false, items=[];
    ...
    document.querySelector('#newAccount').onclick=()=>{form.reset();form.id.value='';form.include_in_net_worth.checked=true;document.querySelectorAll('.create-only').forEach(x=>x.classList.remove('d-none'));form.querySelector('.modal-title').textContent='Add account';M.modal('accountModal');};
    ...
    cards.onclick=async e=>{const edit=e.target.closest('.edit-account'), archive=e.target.closest('.archive-account'); if(edit){const a=items.find(x=>String(x.id)===edit.dataset.id);form.reset();form.id.value=a.id;form.name.value=a.name;form.type.value=M.typeValue(a.type);form.institution.value=a.institution||'';form.last_four.value=a.last_four||'';form.include_in_net_worth.checked=!!a.include_in_net_worth;document.querySelectorAll('.create-only').forEach(x=>x.classList.add('d-none'));form.querySelector('.modal-title').textContent='Edit account';M.modal('accountModal');} if(archive && confirm('Archive this account?')){try{await M.request(`/ajax/accounts/${archive.dataset.id}`,{method:'DELETE'});M.toast('Account archived.');load();}catch(err){M.toast(err.message,'danger');}}};
    form.onsubmit=async e=>{e.preventDefault();M.clearErrors(form);const id=form.id.value, data=M.payload(form);delete data.id; if(id) delete data.type; try{await M.request(id?`/ajax/accounts/${id}`:'/ajax/accounts',{method:id?'PATCH':'POST',body:JSON.stringify(data)});M.modal('accountModal','hide');M.toast(id?'Account updated.':'Account created.');load();}catch(err){M.errors(form,err);}};
    load();
})();
</script>
@endpush
