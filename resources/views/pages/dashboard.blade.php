    ...
    <script>
        (async () => {
            ...
            try {
                ...
                const healthBadge = health.score >= 70 ? 'text-success' : health.score >= 40 ? 'text-warning' :
                    'text-danger';

                const stats = [
                    ['bi-wallet2', 'Net worth', data.net_worth.net_worth, 'Assets minus liabilities'],
                    ['bi-arrow-down-left', 'Income this month', data.month.income,
                        `${data.month.savings_rate}% savings rate`
                    ],
                    ['bi-arrow-up-right', 'Expenses this month', data.month.expense,
                        `${M.date(data.month.from)} – ${M.date(data.month.to)}`
                    ],
                    ['bi-shield-check', 'Financial Health', `${health.score}%`,
                        `${health.emergency_months} mo. emergency fund`
                    ],
                ];
        ...
        `;
                ...
                // Recent Transactions
                const icon = t => {
                    const type = M.typeValue(t.type);
                    return type === 'income' ? 'arrow-down-left' : type === 'expense' ? 'arrow-up-right' : type
                        .includes('loan') ? 'cash-stack' : 'arrow-left-right';
                };
                const cls = t => M.typeValue(t.type) === 'income' ? 'money-positive' : M.typeValue(t.type) ===
                    'expense' ? 'money-negative' : '';
                document.querySelector('#recentTransactions').innerHTML = data.recent_transactions.length ? data
        ...
        `).join('') : '<div class="empty-state py-4"><i class="bi bi-receipt"></i>No transactions yet.</div>';
                ...
                // Budget Summary
                document.querySelector('#budgetSummary').innerHTML = data.budgets.length ? data.budgets.slice(0, 5)
                    .map(b => `
            <div class="mb-3">
