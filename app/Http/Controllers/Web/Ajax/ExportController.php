...
class ExportController extends Controller
{
    public function transactionsCsv(Request $r): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($q) {
            ...
            fputcsv($h, ['Date', 'Reference', 'Type', 'Amount', 'Category', 'Person', 'From', 'To', 'Description', 'Status']);
            $q->chunk(500, function ($rows) use ($h) {
                foreach ($rows as $t)
                    fputcsv($h, [$t->transaction_date->toDateString(), $t->reference_no, $t->type->value, $t->amount, $t->category?->name, $t->person?->name, $t->sourceAccount?->name, $t->destinationAccount?->name, $t->description, $t->status]);
            });
            fclose($h);
