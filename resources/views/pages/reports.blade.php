        ...
        <section class="surface-card">
            <div class="card-header-clean p-3 border-bottom">
                <ul class="nav nav-pills flex-wrap gap-1" id="accountingReportTabs">
                    <li class="nav-item">
                        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tabTrialBalance" type="button">
                    ...
                    <li class="nav-item">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabProfitLoss" type="button">
                            <i class="bi bi-graph-up-arrow me-1"></i>Profit & Loss (P&L)
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabGeneralLedger" type="button">
                            <i class="bi bi-journal-bookmark me-1"></i>General Ledger
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabPartyLedger" type="button">
                            <i class="bi bi-person-lines-fill me-1"></i>Party Statement
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabComparison" type="button">
                    ...
                    <li class="nav-item">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabCommitteesReport" type="button">
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade show active" id="tabTrialBalance">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <span class="text-secondary small">Verifies that total debits equal total credits across all ledger accounts.</span>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                ...
                                <tbody id="trialBalanceTableBody">
                                    <tr><td colspan="5" class="text-center text-secondary py-3">Loading Trial Balance...</td></tr>
                                </tbody>
                    ...
                    <div class="tab-pane fade" id="tabBalanceSheet">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <span class="text-secondary small">Comprehensive financial position verifying <strong>Assets = Liabilities + Equity</strong> with prior period comparison.</span>
                    ...
                    <div class="tab-pane fade" id="tabProfitLoss">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <span class="text-secondary small">Statement of financial performance comparing current period against prior period of equal length.</span>
                        </div>
                        <div id="pnlContent">
                    ...
                    <div class="tab-pane fade" id="tabGeneralLedger">
                        <div class="row g-3 mb-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                <button class="btn btn-primary" id="btnLoadGL"><i class="bi bi-search me-1"></i>View Statement</button>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                ...
                                <tbody id="generalLedgerTableBody">
                                    <tr><td colspan="6" class="text-center text-secondary py-3">Select an account above to view statement.</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="tabPartyLedger">
                        <div class="row g-3 mb-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                <button class="btn btn-primary" id="btnLoadPartyLedger"><i class="bi bi-journal-text me-1"></i>Load Party Statement</button>
                            </div>
                        </div>
                        <div id="partyLedgerContainer">
                            <div class="text-center py-4 text-secondary">Select a person above to view their dedicated account statement.</div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="tabComparison">
                        <div class="row g-3 mb-3 align-items-center">
                            ...
                            <div class="col-md-3">
                                <button class="btn btn-primary mt-4" id="btnLoadComparison"><i class="bi bi-arrow-repeat me-1"></i>Calculate Growth</button>
                            </div>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody id="commReportTableBody">
                                    <tr><td colspan="6" class="text-center text-secondary py-3">Loading committees report...</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
...
(() => {
    ...
    async function loadAccountsSelect() {
        try {
            ...
            M.fillSelect(document.querySelector('#glAccountSelect'), accounts, { placeholder: 'Select account for GL statement...' });
        } catch(e) {}
    }
    ...
    async function loadPeopleSelect() {
        try {
            ...
            M.fillSelect(document.querySelector('#partySelect'), people, { placeholder: 'Select person / party...' });
        } catch(e) {}
    }
    ...
    async function loadBalanceSheet() {
        try {
            ...
            badge.textContent = bs.totals.is_balanced ? 'Equilibrium Verified (Assets = Liabilities + Equity)' : 'Unbalanced';
    ...
    async function loadPartyLedger() {
        ...
        try {
            const pl = await M.request(`/ajax/reports/party-ledger?person_id=${personId}&from=${from.value}&to=${to.value}`);
    ...
    async function loadGeneralLedger() {
        ...
        try {
            const gl = await M.request(`/ajax/reports/general-ledger?account_id=${accountId}&from=${from.value}&to=${to.value}`);
        ...
        try {
            ...
            trendChart = new Chart(document.querySelector('#trendChart'), {
                ...
                data: {
                    ...
                    datasets: [
                        { label: 'Income', data: trend.map(x => x.income), tension: .35 },
                        { label: 'Expense', data: trend.map(x => x.expense), tension: .35 }
                    ]
                },
                options: {
                    responsive: true,
            ...
            categoryChart = new Chart(document.querySelector('#categoryChart'), {
                ...
                data: {
                    ...
                    datasets: [{ data: cats.map(x => x.total) }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
            ...
            budgetChart = new Chart(document.querySelector('#budgetChart'), {
                ...
                data: {
                    ...
                    datasets: [
                        { label: 'Budget Target', data: (budgets || []).map(b => b.amount), backgroundColor: 'rgba(40, 95, 115, 0.55)' },
                        { label: 'Spent Amount', data: (budgets || []).map(b => b.spent || 0), backgroundColor: 'rgba(220, 53, 69, 0.75)' }
                    ]
                },
                options: {
                    responsive: true,
            ...
            if (!commList.length) {
                commTbody.innerHTML = `<tr><td colspan="6" class="text-center text-secondary py-3">No committees found.</td></tr>`;
            } else {
