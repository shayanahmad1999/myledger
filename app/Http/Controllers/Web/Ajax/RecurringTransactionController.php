...
class RecurringTransactionController extends Controller
{
    ...
    private function validateReferences(Request $request, array $data): void
    {
        ...
        if ($type === TransactionType::Expense->value) {
            if (empty($data['source_account_id'])) $errors['source_account_id'][] = 'A source account is required for a recurring expense.';
            if (empty($data['category_id'])) $errors['category_id'][] = 'An expense category is required.';
            if ($category && $category->type !== 'expense') $errors['category_id'][] = 'The selected category must be an expense category.';
        } elseif ($type === TransactionType::Income->value) {
            if (empty($data['destination_account_id'])) $errors['destination_account_id'][] = 'A destination account is required for recurring income.';
            if (empty($data['category_id'])) $errors['category_id'][] = 'An income category is required.';
            if ($category && $category->type !== 'income') $errors['category_id'][] = 'The selected category must be an income category.';
        } elseif ($type === TransactionType::Transfer->value) {
            if (empty($data['source_account_id'])) $errors['source_account_id'][] = 'A source account is required for a recurring transfer.';
            if (empty($data['destination_account_id'])) $errors['destination_account_id'][] = 'A destination account is required for a recurring transfer.';
