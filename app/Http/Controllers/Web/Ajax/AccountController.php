...
class AccountController extends Controller
{
    ...
    public function store(Request $request, LedgerService $ledger): JsonResponse
    {
        ...
        $account=DB::transaction(function() use($request,$data,$ledger){
            ...
            abort_unless($data['currency_id'], 422, 'Base currency is not configured.');
            abort_unless((int) $data['currency_id'] === (int) $baseCurrencyId, 422, 'Multi-currency conversion is not enabled yet. Create accounts in your base currency.');
    ...
    public function update(Request $request, LedgerAccount $account): JsonResponse
    {
        ...
        $data=$request->validate(['name'=>'sometimes|required|string|max:120','institution'=>'nullable|string|max:120','last_four'=>'nullable|digits:4','icon'=>'nullable|string|max:50','color'=>'nullable|string|max:20','include_in_net_worth'=>'sometimes|boolean','is_archived'=>'sometimes|boolean']);
        $account->update($data); return response()->json($account);
    }
