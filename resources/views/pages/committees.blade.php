    ...
    <div class="row g-3 mb-4" id="statsContainer">
        <div class="col-sm-6 col-xl-3">
            <div class="surface-card p-3 d-flex align-items-center gap-3">
                <div class="rounded-circle p-3 text-primary bg-primary-subtle fs-4 d-flex align-items-center justify-content-center"
        ...
        <div class="col-sm-6 col-xl-3">
            <div class="surface-card p-3 d-flex align-items-center gap-3">
                <div class="rounded-circle p-3 text-success bg-success-subtle fs-4 d-flex align-items-center justify-content-center"
                    style="width:52px;height:52px;">
                    <i class="bi bi-cash-stack"></i>
                </div>
                <div>
                    <div class="text-secondary small fw-medium">Monthly Contribution</div>
                    <div class="h4 mb-0 fw-bold" id="statMonthlyContribution">
        ...
        <div class="col-sm-6 col-xl-3">
            <div class="surface-card p-3 d-flex align-items-center gap-3">
                <div class="rounded-circle p-3 text-info bg-info-subtle fs-4 d-flex align-items-center justify-content-center"
                    style="width:52px;height:52px;">
        ...
        <div class="col-sm-6 col-xl-3">
            <div class="surface-card p-3 d-flex align-items-center gap-3">
                <div class="rounded-circle p-3 text-warning bg-warning-subtle fs-4 d-flex align-items-center justify-content-center"
                    style="width:52px;height:52px;">
                ...
                <div class="modal-body">
                    <div class="row g-3 mb-3">
                        <div class="col-md-5">
                            ...
                            <input class="form-control" name="name" placeholder="e.g. Office Monthly Committee 2026"
                                required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Contribution / Member <span class="text-danger">*</span></label>
                            <input class="form-control" name="contribution_amount" id="committeeContributionInput"
                                type="number" min="1" step="0.01" placeholder="5000" required>
                        </div>
                        <div class="col-md-4">
                    ...
                    <div class="mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <label class="form-label mb-0 fw-bold">Member Slots (Minimum 2) <span
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    <div
                        class="bg-light p-3 border-bottom d-flex flex-wrap gap-4 align-items-center justify-content-between">
                        <div class="d-flex gap-4">
                            <div>
                                <div class="text-secondary small">Contribution / Member</div>
                                <div class="fw-bold" id="detailContribution">
                                    {{ auth()->user()->settings->currency->symbol }} 0</div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="fw-bold text-success" id="detailPoolAmount">
                                    {{ auth()->user()->settings->currency->symbol }} 0</div>
                            </div>
                            <div>
                    ...
                    <div class="p-3">
                        <ul class="nav nav-tabs mb-3" id="detailTabs" role="tablist">
                            <li class="nav-item">
                                <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tabRounds"
                                    type="button">
                                    <i class="bi bi-calendar-check me-1"></i>Rounds & Payouts Schedule
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabMembers"
                                    type="button">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info py-2 mb-3 d-flex align-items-center justify-content-between">
                        <div><i class="bi bi-info-circle me-2"></i>Select an account below if you wish to automatically
                            create a ledger transaction for paid contributions.</div>
                    </div>
                    <div class="mb-3">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form class="modal-content" id="payoutForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Turn Recipient (Winning Member) <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control" name="payout_date" type="date"
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-success"><i class="bi bi-check-circle me-1"></i>Confirm & Mark
                        Paid</button>
                </div>
            </form>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="border rounded-4 p-4 bg-light text-center" id="receiptCardContent">
                        ...
                        <div class="h3 fw-bold text-success mb-3" id="receiptAmount">
                            {{ auth()->user()->settings->currency->symbol }} 0</div>
    ...
    <script>
        (() => {
            ...
            async function loadRefs() {
                try {
                    ...
                    M.fillSelect(formCommittee.my_person_id, people, {
                        placeholder: 'Select yourself (Optional)'
                    });
                    ...
                    M.fillSelect(formCommittee.currency_id, currencies, {
                        label: c => `${c.code} (${c.symbol})`,
                        placeholder: 'Select currency...',
                        selected: getBaseCurrencyId()
                    });
                    M.fillSelect(document.querySelector('#paymentAccountId'), accounts, {
                        placeholder: 'Select account for deposit...'
                    });
                    M.fillSelect(payoutForm.account_id, accounts, {
            ...
            async function loadCommittees() {
                try {
                    ...
                    document.querySelector('#statMonthlyContribution').textContent = M.money(stats
                        .total_monthly_contribution || 0, stats.currency.symbol);
        ...
        `;
            ...
            grid.onclick = async (e) => {
                ...
                if (btnDelete) {
                    if (confirm(
                            'Are you sure you want to delete this committee? All round records will be removed.'
                            )) {
                        try {
                            await M.request(`/ajax/committees/${btnDelete.dataset.id}`, {
                                method: 'DELETE'
                            });
            ...
            async function openDetailModal(id) {
                try {
                    ...
                    document.querySelector('#detailModalSubtitle').textContent =
                        `Started ${M.date(currentCommittee.start_date)} · Status: ${currentCommittee.status.toUpperCase()}`;

                    document.querySelector('#detailContribution').textContent = M.money(currentCommittee
                        .contribution_amount, currentCommittee.currency.symbol);
                    document.querySelector('#detailMembersCount').textContent = currentCommittee.total_members;
                    document.querySelector('#detailPoolAmount').textContent = M.money(currentCommittee
                        .total_pool_amount, currentCommittee.currency.symbol);
            ...
            function showReceiptModal({
                ...
                document.querySelector('#receiptAmount').textContent = M.money(amount, currentCommittee.currency
                .symbol);
                ...
                document.querySelector('#btnCopyWhatsApp').onclick = () => {
                    navigator.clipboard.writeText(rawText).then(() => {
                        M.toast('Receipt text copied to clipboard! Ready to paste into WhatsApp.',
                            'success');
                    }).catch(() => {
        ...
        `).join('');
            ...
            function openRoundPaymentsModal(roundId) {
                ...
                document.querySelector('#roundPaymentsTitle').textContent =
                    `Record Contributions - Round #${currentRound.round_number} (${M.date(currentRound.due_date)})`;
            ...
            `;
                ...
                tbody.querySelectorAll('.btn-toggle-payment').forEach(btn => {
                    btn.onclick = async () => {
                        ...
                        try {
                            await M.request(
                                `/ajax/committees/rounds/${currentRound.id}/members/${memberId}/payment`, {
                                    method: 'POST',
                                    body: JSON.stringify({
                                        status: newStatus,
                                        paid_at: newStatus === 'paid' ? new Date()
                                            .toISOString().slice(0, 10) : null,
                                        account_id: accountId || null
                                    })
                                });

                            M.toast(newStatus === 'paid' ? 'Payment recorded as paid!' :
                ...
                tbody.querySelectorAll('.btn-payment-receipt').forEach(btn => {
                    btn.onclick = () => {
                        const p = currentRound.payments.find(x => String(x.committee_member_id) === String(
            ...
            function openPayoutModal(roundId) {
                ...
                document.querySelector('#payoutAmountDisplay').value = M.money(currentRound.payout_amount,
                    currentCommittee.currency.symbol);
