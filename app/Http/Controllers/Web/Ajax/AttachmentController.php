...
class AttachmentController extends Controller
{
    public function store(Request $r): JsonResponse
    {
        ...
        if (!empty($d['financial_transaction_id'])) {
            abort_unless(FinancialTransaction::findOrFail($d['financial_transaction_id'])->user_id === $r->user()->id, 403);
        }
        if (!empty($d['loan_id'])) {
            abort_unless(Loan::findOrFail($d['loan_id'])->user_id === $r->user()->id, 403);
        }
        $file = $r->file('file');
        $disk = config('finance.attachment_disk', 'local');
        $path = $file->store('finance/' . $r->user()->id . '/attachments', $disk);
    ...
    public function download(Request $r, Attachment $attachment): StreamedResponse
    {
        $this->owned($r, $attachment);

        return Storage::disk($attachment->disk)->download($attachment->path, $attachment->original_name);
    }

    public function destroy(Request $r, Attachment $attachment): JsonResponse
    {
        $this->owned($r, $attachment);
    ...
    private function owned(Request $r, Attachment $m): void
    {
