...
class CategoryController extends Controller
{
    ...
    {
        return response()->json(Category::forUser($request->user()->id)->with('children')->when($request->type, fn($q, $v) => $q->where('type', $v))->where('is_active', true)->orderBy('name')->get());
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (!empty($data['parent_id'])) {
            $p = Category::findOrFail($data['parent_id']);
            abort_unless($p->user_id === $request->user()->id && $p->type === $data['type'], 422, 'Parent category must belong to you and have the same type.');
        }
    ...
    public function update(Request $request, Category $category): JsonResponse
    {
        ...
        $data = $request->validate(['name' => 'sometimes|required|string|max:100', 'icon' => 'nullable|string|max:50', 'color' => 'nullable|string|max:20', 'is_active' => 'sometimes|boolean']);
        DB::transaction(function () use ($category, $data) {
            $category->update($data);
            if (isset($data['name']))
                $category->ledgerAccount()->update(['name' => $data['name']]);
        });
    ...
    private function owned(Request $r, Category $m): void
    {
