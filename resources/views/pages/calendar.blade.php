        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="p-3 bg-light rounded mb-3 text-center">
                    <div class="h3 fw-bold mb-1" id="eventModalAmount">Rs 0</div>
...
(() => {
    ...
    async function loadEvents() {
        ...
        try {
            events = await M.request(`/ajax/calendar-events?start=${start.toISOString().slice(0,10)}&end=${end.toISOString().slice(0,10)}`);
    ...
    function renderCalendarGrid(year, month) {
                            ...
                            <div class="d-flex flex-column gap-1 overflow-auto" style="max-height: 80px;">
                                ${dayEvents.slice(0, 3).map(ev => `
                                    <div class="badge bg-${ev.color}-subtle text-${ev.color} border text-truncate text-start calendar-event-item p-1" data-id="${ev.id}" style="cursor:pointer; font-size:11px;" title="${M.esc(ev.title)}">
                                        ${M.esc(ev.title)} (${M.money(ev.amount)})
                                    </div>
                                `).join('')}
        ...
        gridBody.querySelectorAll('.calendar-event-item').forEach(el => {
            el.onclick = () => {
                ...
                document.querySelector('#eventModalAmount').textContent = M.money(ev.amount);
                document.querySelector('#eventModalDate').textContent = `${M.date(ev.date)} · ${ev.type.toUpperCase()}`;
                ...
                document.querySelector('#eventModalDetails').innerHTML = detailsHtml || '<div class="p-3 text-center text-secondary small">No additional details available.</div>';
