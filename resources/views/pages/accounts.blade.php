...
@section('content')
<div class="page-actions justify-content-between" data-base-currency-id="{{ auth()->user()->settings?->base_currency_id }}">
...
(() => {
    ...
    async function refs(){ try { currencies=await M.getCurrencies(); M.fillSelect(form.currency_id, currencies, { label: c=>`${c.code} (${c.symbol})`, selected: getBaseCurrencyId() }); updateAccountRateNotice(); } catch(e){} }
    function getBaseCurrencyId() { return document.querySelector('.page-actions')?.dataset?.baseCurrencyId || null; }
